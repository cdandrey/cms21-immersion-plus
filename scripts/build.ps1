...
function Find-CMS21ImmersionPlusProject {
    $preferredPaths = @(
        (Join-Path $sourceRoot "CMS21ImmersionPlus.csproj"),
        (Join-Path $repoRoot "CMS21ImmersionPlus.csproj")
    )
    ...
    $found = Get-ChildItem -LiteralPath $repoRoot -Filter "CMS21ImmersionPlus.csproj" -File -Recurse -ErrorAction SilentlyContinue |
        Where-Object {
...
$projectFile = Find-CMS21ImmersionPlusProject
...
if ([string]::IsNullOrEmpty($projectFile)) {
    throw "CMS21ImmersionPlus.csproj was not found. Expected '$sourceRoot\CMS21ImmersionPlus.csproj' or '$repoRoot\CMS21ImmersionPlus.csproj'."
}

Assert-ProjectLibraries -ProjectFile $projectFile

$projectDirectory = Split-Path -Parent $projectFile
$solutionFile = Join-Path $projectDirectory "CMS21ImmersionPlus.sln"
$buildInput = if (Test-Path -LiteralPath $solutionFile -PathType Leaf) { $solutionFile } else { $projectFile }
$outputDll = Join-Path (Join-Path $projectDirectory "bin\$Configuration") "CMS21ImmersionPlus.dll"
...
if (-not (Test-Path -LiteralPath $outputDll -PathType Leaf)) {
    throw "Build succeeded, but CMS21ImmersionPlus.dll was not found at the expected path: $outputDll"
}
